...
butterfly apply cost ÷ plain low-rank cost (matched certified accuracy)</text>
    ...
    return page(
        ...
        [
            ...
            "coarsens the column tree. That keeps every factor at rank "
            "about R/2<sup>L</sup>, and its transfer cost does not "
            "depend on N at all. So at fixed R the cost ratio must "
...
<path d="{ax.path(ws, exact)}" fill="none" class="board-ink"
    ...
    return page(
        ...
        [
            ...
            "found. Suppose the reconstructed kernel deviates from the "
            "target Gaussian by less than c·e^(−ω) everywhere. That "
            "bound is checked on a dense grid, with a Lipschitz "
            "estimate per cell and an analytic tail. Then positivity "
            "of ρ gives |value − truth| ≤ c·C(1). The one physical "
...
<text x="{ax.ml + 10}" y="{ax.mt + 14}" class="board-text" font-size="10.5"
 ...
 (rust)</text></svg>'''
    ...
    return page(
        ...
        "certified case",
        "A breaking wave against a sea wall",
        ...
        [
            ...
            "converges and certifies, with a wide error bar. This "
            "laptop-budget model is below its asymptotic range, and "
            "the certificate says by how much.</p>"
            ...
            "asymptotic range. It also caps the usable order at the "
            ...
            "hit and measured during this build.</p>"
            ...
            "second order, and the shortfall comes from the impact and "
            "the boundary treatment. The peak points fall on no slope "
            ...
            "order is in the provenance. The finer verification rung "
            "the certificate never saw lands "
...
<text x="{ax.ml + 10}" y="{ax.Y(max(tight["m_cal"], 1e-17)) - 8:.1f}"
 ...
 calibrated {tight["m_cal"]:.0e}, the manifold's true rank from data</text>
</svg>'''
...
{"".join(np_)}</svg>'''
    ...
    return page(
        ...
        [
            ...
            "FF′(ψ) is weighted Poisson with κ = 1/R. For elliptic "
            "problems there is a bound that is guaranteed rather than "
            "estimated. The Prager-Synge identity says this. Take any "
            "vector field σ with square-integrable divergence. The "
            "energy-norm error of a computed ψ_h is then at most "
            "‖κ∇ψ_h + σ‖ plus a divergence-mismatch term, and the "
            "constant in that term is the rectangle's exact first "
            "eigenvalue. FEniCSx proposes. It solves the primal "
            "problem, and a mixed Raviart-Thomas problem to get a good "
            "σ. The bound holds for whatever it returns. During this "
            "build a sign error in the mixed boundary term produced a σ "
            "that inflated the bound to 27 times the true error, and "
            "the bound was still valid. Once the sign was fixed it sat "
            "at 1.6 times. A bad guess makes the bound looser. It "
            "cannot make the bound wrong.</p>"
            "<p>The implicit coupling is the interesting part. With a "
            "source term c·ψ, the problem the solver iterates on "
            "depends on its own answer. The certificate handles that "
            "without trusting the iteration. The coupled solution map "
            "is a contraction with factor θ = c·R_max/(R_min·λ₁), where "
            "λ₁ is again rectangle-exact. The frozen-source bound "
            "divided by (1−θ) then covers the true coupled solution. At "
            "θ ≥ 1 the function refuses, because past that limit the "
            "fixed point is not certifiably unique.</p>",
            ...
            f"{'refused' if refused else 'NOT REFUSED'}</strong>.</li>"
            "<li>The flux exports its sensitivity to the source. It is "
            "&#8730;area&#183;R<sub>max</sub>/(&#955;&#8321;&#183;"
            "R<sub>min</sub>&#183;(1&#8722;&#952;)) = "
            f"{sens.bound:.3g} per unit of source in L&#178;, RIGOROUS. "
            "That is the contraction constant repriced as a Lipschitz "
            "bound for composed plans. This run exercises it. A "
            "constant source offset (&#8214;&#948;&#8214; = 1) moved "
            f"the flux by {smoved:.3g} against a budget of "
            f"{sbudget:.3g}: "
            ...
            "<p>Everything above needs two things. The source must be "
            "linear in ψ, because that is what the Picard contraction "
            "closes over, and θ must be below 1. A real pressure "
            "profile gives neither. Tokamak pressure peaks on the "
            "magnetic axis and falls away outward, so p′ "
            "<em>decreases</em> with ψ, and the source is a nonlinear "
            "function of the unknown.</p>"
            "<p>Two things then break, and they break independently. "
            "The contraction factor is max|S′|·R_max/(R_min·λ₁). For an "
            f"exponential profile of decay scale ψ₀ = {PSI0:g} that "
            "maximum is c/ψ₀ rather than c. A peaked profile is a steep "
            f"one, so the wall arrives five times sooner, at "
            f"c = {c_wall:.3g} instead of 2.34.</p>"
            "<p>The second break is structural, and it does not care "
            "about c at all. Differentiating the source contributes "
            "−S′ times the consistent mass matrix. That matrix has "
            "off-diagonal entries which are integrals of products of "
            "non-negative basis functions, so they are "
            "<em>positive</em>. A decreasing profile has S′ &lt; 0, so "
            "those positive entries land on top of the stiffness "
            f"matrix's negative ones. At n = 8 and a coupling of "
            f"c = 0.1, {npos} of them come out positive, at a θ of "
            f"{nz['theta']:.2g}. That is well inside what the "
            "contraction would have accepted. The Jacobian is no longer "
            "a Z-matrix, so the reactor's cone witness cannot price "
            "‖J⁻¹‖ here. The shape of the profile closed that door, not "
            "the strength of the coupling.</p>"
            "<p>What answers is the same Kantorovich theorem the "
            "junction runs on, taking the other route to ‖J⁻¹‖. Pick "
            "any approximate inverse R. If ‖I − RJ‖ &lt; 1 then the "
            "Neumann series converges, J is nonsingular, and "
            "‖J⁻¹‖ ≤ ‖R‖/(1 − ‖I − RJ‖). That asks nothing about signs. "
            "It costs a matrix inverse rather than a solve. Where the "
            "cone route also applies, the two agree to every digit "
            "printed, because on an M-matrix the witness is not merely "
            "valid but exact. J⁻¹ ≥ 0 makes ‖J⁻¹‖<sub>∞</sub> literally "
            "‖J⁻¹e‖<sub>∞</sub>, which is what the witness "
            "computes.</p>"
            ...
            "apply, use Prager-Synge. Where it refuses, a discrete "
            "answer with a stated mesh gap is better than no "
            "answer.</p>",
            ...
            "it. The Kantorovich radius keeps shrinking as the coupling "
            "grows, because a stiffer problem has a smaller ‖J⁻¹‖. It "
            f"stays {min(gaps):.0f} to {max(gaps):.0f} orders below the "
            "discretization, which is what actually binds. The "
            "certificate is about the discrete equilibrium. The upper "
            "curve is the distance from there to the differential "
            "equation, and it is measured rather than "
            "carried.</figcaption></figure>"
            ...
            f"{zmat['route']}</strong>. The stiffness matrix alone is a "
            "Z-matrix, so the cheap route answers. With the profile "
            f"switched on at c = 0.1: <strong>{nz['route']}</strong>."
            "</li>"
            f"<li>Discretization is second order in h, measured "
            "8 → 16 → 32. The certified radius stays below a millionth "
            "of it at every mesh, so what limits this answer is the "
            "model boundary and not the arithmetic.</li>"
            "<li>Assembly quadrature is not carried, which is the same "
            ...
            "<p>The comparison above was never quite like for like, so "
            "here it is stated plainly. Prager-Synge is RIGOROUS about "
            "the <em>continuum</em> equilibrium. Kantorovich is "
            "RIGOROUS about the <em>discrete</em> one. Same tier label, "
            "different questions, and the second is the easier "
            "question.</p>"
            "<p>The missing statement can be bought the same way the "
            "reactor and the junction buy it. Refine the mesh and read "
            "the trend. It costs a tier, because a measured convergence "
            "order is not a proven one.</p>"
            ...
            "problem at every coupling. The continuum flux is then the "
            "integral of a polynomial over a rectangle, and it can be "
            f"done by hand: {gs_truth:.9f}. The certificate contains "
            "it. The true remaining distance is "
            f"{gs_cont.err / abs(gs_cont.value - gs_truth):.2f}× inside "
            "the claimed budget, which is the grid certificate's "
            "declared safety factor of 3, arrived at from the other "
            "direction.</p>"
            "<p>Compare the two error bars before deciding which "
...
<text x="{ax.ml + 10}" y="{320 - ax.mb - 10}" class="board-text"
    ...
    return page(
        ...
        "ladders. The certificates decide what is true, and the cost "
        "models never do.",
        [
            "<h2>The idea</h2>"
            "<p>Until now every page on this site answered its "
            "question with an algorithm someone chose by hand. This "
            "page is the first piece of the promised compiler, because "
            "the choice itself is made by a program. Each way of "
            "answering a question is declared as a rewrite with a "
            "ladder of effort and a guessed cost per rung. The planner "
            "runs the cheapest promise first. If the certificate that "
            "comes back meets the tolerance, the work is done. If not, "
            "the measurement is kept and the rewrite's next rung goes "
            "back in the queue. After two failures a fitted decay model "
            "picks how far to jump. Then the next cheapest promise "
            "runs, which may belong to a competing rewrite. Nothing "
            "here is trusted. A wrong cost model or a bad jump wastes "
            "some running time. It cannot corrupt the answer, because "
            "every answer still arrives with its own "
            "certificate.</p>",
            ...
            "eigendecompositions times 2<sup>ℓ</sup>. As the tolerance "
            "tightens the planner climbs the ladder. Past the wall the "
            "window widths floor out on the relaxation gap, so the "
            "dense 1024-dimensional bracket wins the competition. That "
            "one is exact, and priced 2<sup>N</sup>. The window band is "
            "short here because ten sites is small enough to "
            "diagonalize. Once the two rewrites are priced in the same "
            "unit, the honest advice at this size is mostly <em>form "
            "the matrix</em>. The map below shows where that stops "
            "being true. The wall belongs to this 10-site chain, and on "
            "a 10⁶-site chain there is no dense column to flee to. "
            "Chosen is also not the same as optimal. The y-axis is the "
            "cheapest plan found among the declared rewrites, not the "
            "cheapest possible algorithm, because the planner's claim "
            "stops at its library.</figcaption></figure>"
            ...
            "the certificate's structured receipt. Here is the tightest "
            "run's receipt, one rung per line:</p>"
            f"<pre>{esc(receipt_lines)}</pre>"
            "<p>Those seconds are honest but local. Another machine, or "
            "this one an hour busier, gives different numbers for the "
            "same work. The tempting fix is a ruler. Divide every "
            "measurement by a fixed microbenchmark timed on the same "
            "machine, and the ratio should travel between machines. "
            "This library shipped that for a few hours and then "
            "measured it. It does not work, at least not with a small "
            "benchmark against these rungs. Under three competing "
            "processes the 256-dimensional benchmark slowed by "
            "480&#215;, while the 1024-dimensional rung it was supposed "
            "to price slowed by 35&#215;. On an idle machine, across "
            "four fresh processes, the rung's raw seconds spanned "
            "1.7&#215; and the same seconds divided by the benchmark "
            "spanned 5.6&#215;. The yardstick moved more than the thing "
            "being measured. A short dense benchmark and a long one do "
            "not share a cost direction. They differ in how much they "
            "depend on cache residency and on getting all their threads "
            "scheduled at once. Dividing by one of them removes one "
            "machine and introduces another.</p>"
            "<p>So the rows carry seconds, and the receipt claims only "
            "what it can support. On <em>this</em> machine, this "
            "prediction went with that measurement. That is still the "
            "pair a cost model learns from, and it is simply learned "
            "per machine. Making predictions portable is a job for a "
            "cost model written over each rewrite's own parameters, "
            "such as dimensions, iteration counts, and the sizes the "
            "algorithm already knows. It is not a job for a yardstick "
            "held up beside them.</p>",
            "<h2>The phase map</h2>"
            "<p>The staircase is one slice of a bigger object. Sweep "
            "both knobs, chain length and tolerance, and the plan "
            "traces assemble a phase map of the query. The map shows "
            "which rewrite is cheapest-found where, and where the "
            "declared library ends. This is what the compiler's answers "
            "look like in bulk. As the rewrite library grows, the map "
            "is the real deliverable, because it gives the certified "
            "boundary of what each method can do rather than one "
            "certified number.</p>"
            ...
            "windows. Rust: the dense 2<sup>N</sup> bracket wins. That "
            "column ends at N = 12, where the matrix stops being "
            "formable. Blank: every declared rung ran and none "
            f"certified, which is {n_refused} of {len(grid)} cells "
            "here, each refusing with a receipt. The boundary of the "
            "blank region is the certified price wall of this library, "
            "and every rewrite added to the module redraws "
            ...
            "<code>eigsh</code>, left to itself, draws its ARPACK start "
            "vector from numpy's global random stream. A bracket's "
            "value therefore depended on how many other calls had run "
            "first in the same process. The &#8467; = 7 window at "
            "N = 10 came back 0.382, 0.332 and 0.379 on three "
            "consecutive calls. The cause is physics, not arithmetic. A "
            "Heisenberg window on an odd number of sites has a "
            "spin-&#189; ground <em>doublet</em>. The measured gap is "
            "4&#215;10<sup>&#8722;16</sup> at &#8467; = 5 and "
            "7&#215;10<sup>&#8722;16</sup> at &#8467; = 7, against "
            "10<sup>&#8722;1</sup> at the even widths. Both members "
            "carry the same energy, but they carry opposite edge spins, "
            "and the edge spin is what the upper bound uses to price "
            "the bond between blocks. This is not a validity question. "
            "That vector is a variational trial state, so an unlucky "
            "member loosens a bracket and cannot invalidate one. An "
            "unrepeatable certificate is the worse failure anyway. The "
            "start is now a fixed seeded draw.</li>"
            ...
            "Above 100% means eighty gives the <em>worse</em> bracket, "
            ...
            "now the default. None of it was measurable until the start "
            "vector above was pinned. The first version of this "
            "measurement read 94.5% here and 51.7% on the CI machine, "
            "and both numbers were the wandering vector rather than the "
            "ascent.</li>"
            ...
            "</strong>, so it is nearly twice as dear rather than half. "
            "The missing term was the multiplier ascent. A window rung "
            "runs it before it brackets anything, and the dense rung "
            "does not run it at all, so the two rewrites that race here "
            "were quoted in different currencies. Both are now priced "
            "in eigendecompositions times dimension.</li>"
            "<li>The same window@2 rung was priced 40 in both plans. It "
            f"measured {esc(w2_cold)} in the first plan and "
            f"{esc(w2_warm)} in the last, because its corrections were "
            "cached by then. Cost is state-dependent and the "
            "predictions are not, and only logging both exposes that. "
            "These pairs are kept in every run, and they are the "
            "calibration data for better cost models on the machine "
            "that produced them.</li>"
            "<li>A negative result, kept because it cost something to "
            "learn. Normalizing those seconds by a fixed microbenchmark "
            "so they would travel between machines was built, shipped, "
            "measured, and removed the same day. The benchmark is "
            "480&#215; contention-sensitive where the rung is 35&#215;, "
            "so the ratio is noisier than the raw seconds it "
            "replaced.</li>"
            ...
            "construction, because plan() returns only what a "
            "certificate approves.</li>"
            "<li>What this is not, yet. The plan space is single-knob "
            "ladders per query. The first composed plan splits one "
            "error budget between two stages, and it now has "
 ...
 &#963; = {sigma:g}</text>
    ...
    try:
        ...
        pipeline = [
            ...
            "debt this page named at the bottom was pipelines of "
            "<em>different</em> rewrites, and it comes due here. The "
            "query is the total poloidal flux of a "
            '<a href="gs-equilibrium.html">tokamak equilibrium</a> '
            "whose current profile is a declared infinite Legendre "
            "series. No solve ever sees that source exactly. Two "
            ...
            "<code>Certified.through</code>. The tier of the result is "
            "the weakest claim anywhere in the chain.</p>",
            code_section(sf.Certified.through, sf.legendre_source_profile,
                         sf.gs_flux_dispatch),
            "<h2>The pipeline's result</h2>"
            "<p>The tol = 0.2 certificate, with its full provenance "
            "stack verbatim. It holds two rewrites' certificates, the "
            f"conversion, and the plan trace:</p><pre>{pipe_stack}</pre>"
            "<ul>"
            f"<li>Both pipeline answers certify their tolerance "
            f"(err {pipe_a.err:.3g} &#8804; 0.5, {pipe_b.err:.3g} "
            "&#8804; 0.2) and stay RIGOROUS through the chain. They "
            "also agree within their joint error, which they must, "
            "since they are two rungs bounding the same full-series "
            f"flux: <strong>{'yes' if pipe_agree else 'NO'}</strong>."
            "</li>"
            ...
            "names the mesh as the wall rather than the profile: "
    ...
    deeper = [
        ...
        "middle. The query is the smeared spectral value of a declared "
        "pole tower, with weight A&#183;&#961;<sup>k</sup> at energy "
        "E&#8320; + k&#183;dE. There are infinitely many states, so no "
        "finite correlator ever holds them all. Three stages answer it. "
        "A model stage truncates the tower at K poles and certifies the "
        "dropped tail exactly, because the tail is a geometric series "
        "and can be summed rather than estimated. A measurement stage "
        "averages m noisy samples of the truncated correlator. The "
        "smearing stage converts correlator into answer through the "
        "kernel. Three bills, one tolerance.</p>"
        "<p>Here is the new thing. The exchange rate between the model "
        "bill and the answer is the smearing certificate's exported "
        ...
        "rung of the resolution ladder reprices the model stage before "
        "it, and the plan's chosen K moves when either the budget or "
        "the kernel moves. That midstream repricing is what makes this "
        "a chain rather than three independent budgets.</p>",
        code_section(sf.pole_correlator, sf.spectral_pipeline_dispatch),
        "<h2>The chain's result</h2>"
        "<p>The tol = 0.24 certificate, with its provenance stack "
        "verbatim. It holds the model's exact tail, the smearing "
        "certificate, the conversion, the three-way split, and the plan "
        "trace:</p>"
        f"<pre>{p3_stack}</pre>"
        "<ul>"
        ...
        "bill as the wall rather than the model: <code>"
    ...
    fan_in = [
        ...
        "six-atom hydrogen chain from d = 1.8 to d = 3.0 bohr. That is "
        "the difference of two ground-state brackets which never see "
        "each other. Nothing converts into anything, and the errors "
        "simply add. The only question left is how to divide one "
        "tolerance between two independent branches.</p>"
        "<p>Here the arithmetic that priced every earlier split runs "
        "out. Those splits worked because a curve was known ahead of "
        "the run, such as a geometric tail summed exactly or a "
        "1/&#8730;m law. The budget could then be divided by formula "
        "before anything was spent. The h-chain window bracket obeys no "
        "such law. What a window of width &#8467; certifies is whatever "
        "the run measures. So the allocation becomes a <em>search</em>. "
        "Every pair of widths is a rung, priced "
        "4<sup>&#8467;near</sup>&nbsp;+&nbsp;4<sup>&#8467;far</sup>, "
        "and the planner walks the product ladder in cost order until a "
        "pair certifies. Same frontier, same referee, different shape "
        "of plan.</p>",
        ...
        "predicted cost. Rows are the compressed branch and columns the "
        "stretched one. The grid is visibly not symmetric, because the "
        "two branches are not equally hard. Outlined cells are the "
        "pairs chosen at tol = 0.5 and tol = 0.3.</figcaption></figure>"
        "<p>The compressed chain delocalizes across all six atoms, so a "
        "window of width &#8467; misses more of it. The stretched chain "
        "is nearly decoupled, and a window of the same width catches "
        "almost everything. Measured, the compressed bracket "
        ...
        ", with &#8467; = 4 on the compressed branch and 3 on the "
        "stretched, at a predicted cost of 320. The cheapest pair a "
        "single shared knob could reach is &#8467; = 4 on both, "
        "predicted 512, which is <strong>1.6&#215;</strong> the price "
        "for the same tolerance.</li>"
        ...
        "<strong>8</strong> brackets, which is two branches times four "
        ...
        " s.</li>"
        "<li>Past the pair floor the refusal names <em>which branch is "
        "the wall</em>. With two branches, the question of what to buy "
        "next has two answers, and only one of them helps: <code>"
        f"{esc(str(g_refusal)[-190:])}</code></li>"
        "<li>An honest wrinkle. Both runs enclose the same true gap, "
        "so their brackets must overlap, and they do. But the narrower "
        f"one is not the stronger statement. tol = 0.5 gives "
        f"{g_a.value:.3f} &#177; {g_a.err:.3f} and tol = 0.3 gives "
        f"{g_b.value:.3f} &#177; {g_b.err:.3f}. The half-width is "
        "smaller and the midpoint also moved. Whether the gap is "
        "positive is settled by intersecting certified enclosures, not "
        "by keeping the narrowest.</li></ul>",
    ]
    ...
    return page(
        ...
        "of noisy Monte Carlo data. Two error streams must now be paid "
        "out of one tolerance: what the kernel cannot resolve, and what "
        "the noise obscures. The first composed plan splits the budget "
        "by formula, priced by a sensitivity the certificate exports, "
        "and the certificate referees the result.",
        [
            "<h2>The idea</h2>"
            ...
            "so the certificate paid a single bill. That was the "
            "smearing bill c&#183;C(1), which measures what the "
            "reconstructed kernel cannot resolve, and buying more "
            "correlator times N is the only thing that reduces it. Real "
            "correlators come out of Monte Carlo. On a lattice each "
            "C(t) is the mean of m noisy samples, and noise adds a "
            "second bill, z&#183;amp/&#8730;m. Only buying more samples "
            "m reduces that one.</p>"
            "<p>One tolerance must cover both bills, so something has "
            'to decide the split. <a href="the-compiler.html">The '
            "planner page</a> made a cheaper decision, which was simply "
            "which rung of one ladder to run next. This page is the "
            "promised next step. It is a <em>composed</em> plan that "
            "divides one error budget between two stages, resolution "
            "and statistics, before a single sample is bought. The "
            "division itself is near-solved mathematics, the same "
            "marginal-cost balancing that multilevel Monte Carlo uses "
            "to allocate work across levels. Because the statistics "
            "bill obeys a 1/&#8730;m law exactly, the split collapses "
            "to a closed form per rung of the resolution ladder.</p>"
            ...
            "the norm of g. It re-anchors the smearing bill too, and "
            "Cauchy-Schwarz folds both into the one exact constant "
            "&#8730;(c&#178;+&#8214;g&#8214;&#178;), which is rigorous "
            "whatever the tier of the value's own bound. That one "
            "number prices the statistics stage in advance. As "
            "everywhere on this site, the prices only order the "
            "attempts. The certificate of the run that executes is the "
            "referee, so a wrong pilot estimate costs extra rungs and "
            "cannot cost truth.</p>",
            code_section(sf.Sensitivity, sf.smeared_spectral_dispatch),
            "<h2>The result</h2>"
            "<p>The bench is the two-peak density of the earlier page, "
            f"with peaks at &#969; = {E[0]:g} and {E[1]:g} carrying "
            f"weights {a[0]:g} and {a[1]:g}. It is queried at "
            f"&#969; = {omega:g}, &#963; = {sigma:g}, with "
            f"{rel * 100:g}% relative noise per sample. measure(N, m) "
            "returns the mean of m samples, and the declared ladder "
            "offers N = 8, 12, 16 correlator times.</p>"
            ...
            "The plan walks the lower envelope, and the dots mark the "
            "splits it actually chose. At tol = 0.4 the coarse N = 8 "
            "kernel wins by a nose. Tightening the budget hands the "
            "race to N = 12, whose smearing bill is smaller, at ever "
            "larger m.</figcaption></figure>"
            ...
            *fan_in,
            "<h2>Checked in this run</h2><ul>"
            f"<li>Containment: <strong>{contained}/3</strong> "
            "certificates contain the exact smeared truth. It is "
            "computable because the density is synthetic, and the plan "
            "never uses it.</li>"
            ...
            f"N = {hN} with m = {hm}, at a predicted cost of "
            f"{hcost:g}. The coarse N = 8 kernel would have needed "
            f"m = {m8} samples, predicted {8 * m8:.3g}, which is "
            ...
            "and all four are wired through it. A plan is declared as "
            "stages plus one assemble function. Each stage names the "
            "stages it consumes, so the object is a graph, and assemble "
            "joins the certificates with <code>through</code>, "
            ...
            "solved, in the order the budget is spent, and a stage that "
            "cannot is enumerated and walked cheapest-first. The fan-in "
            "uses two searched stages joined by subtraction. The "
            "equilibrium pipeline uses one enumerated mesh and one "
            ...
            "CHOICE rather than a certificate. An N-point kernel spends "
            "budget on what it cannot resolve, and that bill lands "
            "inside the measurement's certificate rather than in one of "
            "its own. Two stages may also share that choice, which is "
            "how the pole model and the measurement come to live on one "
            "grid.</li>"
            "<li>One sentence stopped being hand-written. Every front "
            "door used to name its own binding stage in the refusal, so "
            "it could only ever describe the shape its author had in "
            "mind. The combinator derives it instead. Zero each stage's "
            "error in turn, reassemble, and see which one the composed "
            "error was leaning on. Along a line that drop is the error "
            "converted through the sensitivity. At a fan-in it is the "
            "branch's own error. The assembly does the arithmetic "
            "either way, and it costs nothing, because every "
            "certificate is already in hand.</li>"
            "<li>Going back is not a matter of discipline. A rewrite "
            "whose knob is a tuple is an assignment across stages "
            "rather than a ladder, and the planner refuses one it did "
            "not build itself. The build in turn refuses any front door "
            "that switches that check off, so a release cannot ship the "
            "debt back.</li>"
            "<li>The walk is the cost ladder, and it stays that way on "
            "evidence rather than on inertia. Three searches that use "
            "the graph's shape instead have been built and abandoned. "
            "<em>Escalate the binding stage.</em> This reached a "
            "certifying pair in 4&ndash;7 runs against the ladder's "
            "8&ndash;16. But the thing that costs is nodes computed "
            "rather than assemblies, because the branches are shared. "
            "Counted that way it was 5 against 6, then 6 against 6, "
            "then 7 against 6, and twice it returned a dearer "
            "assignment, because it never revisited what it stepped "
            "over. <em>Order by marginal node cost,</em> so that no "
            "node is bought while a free assembly remains. This saved "
            "one node at one tolerance in eight, and returned a dearer "
            "assignment there. <em>Prune assignments whose measured "
            "components already exceed the tolerance.</em> This one is "
            "admissible and gives the same answer every time, and it "
            "cuts a five-branch walk from 1024 assemblies to 5. It "
            "still saves no nodes at all in nine of twelve cases, from "
            "two branches up to five.</li>"
            ...
            "are arithmetic on certificates already in hand. Beating it "
            "would take a graph whose cheap rungs did not already cover "
...
<text x="{ax.ml + 6}" y="{ax.h - 4}" class="board-text" font-size="10.5"
    ...
    return page(
        ...
        [
            ...
            "There is no minimum to overshoot. Handing the matrix to a "
            "dense eigensolver does not rescue the situation either. A "
            ...
            "checked at runtime rather than assumed.</p>"
            "<ol><li>The loss operator has no positive off-diagonal."
            "</li><li>The fission operator has no negative entry.</li>"
            "<li>The trial flux is strictly inside the cone.</li>"
            "<li>There is a positive vector u with Lu ≥ 1.</li></ol>"
            "<p>The last two do double duty. The vector u proves the "
            "inverse is non-negative, and it <em>also</em> prices the "
            "linear solver's own error, so the bracket stays rigorous "
            "however sloppily the solve was done.</p>",
            ...
            f"{mesh[2][1]:.2f} pcm at the default resolution. That is "
            "about a million times the certificate's own floor. On this "
            ...
            "position. Discretise that instead, with discrete ordinates "
            "on a Gauss-Legendre angular quadrature and streaming "
            "differenced upwind. It changes the operator completely and "
            ...
            + "</tbody></table>"
            "<p>Thousands of pcm wrong in a small leaky slab, and tens "
            "in a large one. That is the textbook behaviour, measured "
            "rather than recited. Neither bracket knows this, and "
            "neither is wrong, because each is rigorous about its own "
            "operator. Which operator deserves belief is a modelling "
            "question, and it stays a separate, stated step.</p>",
            "<h2>Checked in this run</h2><ul>"
...
<text x="{ax.ml + 6}" y="{ax.h - 4}" class="board-text" font-size="10.5"
    ...
    rows = "".join(
        ...
        + (f"<td>{q:.6f}</td><td>{w:.3g}</td></tr>" if w is not None
           else "<td>n/a</td><td>refused</td></tr>")
    ...
    return page(
        ...
        [
            ...
            "solution far away, and with no solution at all. Residuals "
            "are small near almost-singular points too, and that is "
            "precisely the neighbourhood these equations live in.</p>"
            ...
            "negative off-diagonals, and the carrier response "
            "contributes a positive diagonal. That is the same "
            "hypothesis a reactor's loss operator satisfies, so "
            "<code>mmatrix_witness</code>, written for neutrons, prices "
            ...
            "a bound on the inverse Jacobian and does not care where it "
            "came from. It was the <em>pricing method</em> that needed "
            "a Z-matrix. A second route asks nothing about signs. Take "
            "any approximate inverse R. If "
            "&#8214;I &#8722; RJ&#8214; &lt; 1 then the Neumann series "
            "converges and &#8214;J<sup>-1</sup>&#8214; is at most "
            "&#8214;R&#8214;/(1 &#8722; &#8214;I &#8722; RJ&#8214;). It "
            "costs an inverse instead of a solve, so the cone route is "
            "tried first, and it still answers here. On an M-matrix the "
            "cone route is not merely valid but exact, since "
            "J<sup>-1</sup> &#8805; 0 makes "
            ...
            "The solid line is the certified error. It does not exist "
            f"until step {first}, because Kantorovich either closes or "
            "it does not. Then it falls off a cliff, because quadratic "
            "convergence squares the residual and the theorem is linear "
            ...
            "cannot arrive however loose the request. The refusal says "
            "that, rather than reporting the iterate it happened to "
            "reach.</p>",
            ...
            "certificate is nowhere near the weak link. That is only "
            "known because both were measured, and it is the reason to "
            "print both.</p>"
            "<h2>Asking for the junction instead of the mesh</h2>"
            "<p>That gap can be closed, and without a new theorem there "
            "is only one way. Refine the mesh, watch the answer move, "
            "and bound where it is going. Each rung is a certified "
            "discrete answer, and the ladder is read by the same "
            "grid-convergence machinery the reactor and the sea wall "
            "use.</p>"
            ...
            "EMPIRICAL however rigorous each rung was. On the very same "
            f"mesh it is {cont.err / fine400.err:,.0f} times wider than "
            "the rigorous discrete certificate. That is not a defect in "
            "either number. It is the exchange rate between a proven "
            "statement about a model and a measured one about the "
            "world, and it is why this library prints both rather than "
            "quietly reporting the tighter one.</p>"
            "<p>There is no closed form for this junction to check "
            "against, so the check is out of sample. The certificate is "
            "built from meshes no finer than 400 cells. The charge is "
            "then computed at 800 and 1600 cells, which it never saw. "
            "It contains "
            ...
            "0.78&nbsp;nC/cm&#178;, which is thirty-three times too "
            "coarse to read the mesh trend at all, to "
            "1.1&#183;10&#8315;&#8309; in one step. That is two "
            "thousand times tighter than the ladder needs. Every budget "
            "in the range that matters picks the same rung, so there is "
            "no exchange rate to price, and a stage split would be "
            "decoration on a knob that does not respond. The cliff two "
            "sections up is the same fact seen from the other "
            "side.</p>",
            ...
            "clean of carriers, so it must overstate the charge. It "
            "must also overstate it by less as reverse bias widens the "
            ...
            "textbook says it should be. That is the strongest check "
            "available here, because nothing in that formula touched "
            "the matrices.</p>",
            "<h2>The borrowed witness, and the bug it found</h2>"
            ...
            f"{cond:.2g}. The solve is then wrong in the eighth digit, "
            f"and the nudged vector satisfies only Lu &#8805; "
            f"{naive_short:.10f}, which is short of 1. The witness "
            "therefore refused a matrix that is perfectly good.</p>"
            "<p>The fix removes the constant instead of enlarging it. "
            "Scale u by its own measured shortfall. That costs one "
