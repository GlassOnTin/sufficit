...
([`test_sufficit.py`](test_sufficit.py), 208 checks). Every claimed bound is
...
| The 2-RDM lower bound, paired and raced (chemistry) | 2.25 mHa below FCI at H₄ and 7.57 mHa at H₆; H₁₄ certified in 497 s, at 2²⁸ Fock states, where the matrix cannot be formed. Paired with the window ladder's variational upper half, the H₆ bracket at ℓ=2 goes from 387 to 25 mHa per atom | The 2-positivity relaxation contains every N-representable 2-RDM, so its minimum lies below the true one. SCS proposes the multipliers and nothing trusts them: any PSD pair certifies, and a deliberately random pair still bounds, at −2550 against an exact −2.175. Positivity is issued by the library's own eigenvalue bracket, so the certificate itself costs 0.01 s. Spin projection blocks every matrix by S_z without moving the bound, and that is what makes H₁₀ and beyond reachable at all. No exact answer exists out there, so the check is consistency: the per-atom bound reads −0.540842 at H₁₂ and −0.540805 at H₁₄. The two halves must be about the same sector, so the window's trial-state electron count is carried on its certificate and checked, and a mismatch refuses. The planner now races the pair against the window alone, and spends nothing on the program while the cheap rewrite still meets the tolerance |
